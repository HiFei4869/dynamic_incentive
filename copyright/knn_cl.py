...
def compute_average_scores(input_csv_path, top_n_similar_images, output_csv_path):
    ...
    # Ensure the column is named 'combined_loss'
    if 'combined_loss' not in scores_df.columns:
        raise ValueError("Input CSV must have a column named 'combined_loss'.")
    
    average_scores = []

    for target_image, indices in top_n_similar_images.items():
        # Retrieve the scores of the top_n similar images
        similar_scores = scores_df['combined_loss'].iloc[indices].values

        average_score = np.mean(similar_scores)

        average_scores.append(average_score)

    output_df = pd.DataFrame({'copyright_loss': average_scores})
