...
os.environ['DATASET_DIR'] = "../generative-models/artbench_expressionism_200_imagefolder"
os.environ['NUM_TRAIN_EPOCHS'] = "9501"
...
def dataloader_to_dataframe(dataloader):
    ...
    for batch in dataloader:
        ...
        devices_list.append(str(device))  # Store device as string
...
def main(args):
    ...
    return accelerator, unet, vae, optimizer, noise_scheduler, text_encoder_one, text_encoder_two, lr_scheduler, num_update_steps_per_epoch, train_dataloader, params_to_optimize
...
if __name__ == "__main__":
    args = parse_args()
    accelerator, unet, vae, optimizer, noise_scheduler, text_encoder_one, text_encoder_two, lr_scheduler, num_update_steps_per_epoch, train_dataloader, params_to_optimize = main(args)
    ...
    explainer = shap.KernelExplainer(trainer.train, df, keep_index=True)
    ...
    if accelerator.is_main_process:
        ...
        shap_df.to_csv("shap_results.csv")
        shap_raw = pd.DataFrame(numpy_results)
        shap_raw.to_csv("shap_results_raw.csv")
