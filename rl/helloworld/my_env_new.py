...
class BudgetEnv(gym.Env):
    ...
    def process_input(self, input_list, round_num):
        ...
        print(f"round_num:{round_num}, input_list:{input_list}")
        ...
        if input_list == [0,0,0,0,0]:
            return 1000
        for i, budget_distribution in enumerate(input_list):
            ...
            if budget_distribution == 0 and i != 4:
                ...
                # print("inner_train")
                inner_train(round_num, budget_distribution)

        round_dir = f"../../generative-models/rl_logging/Artbench-1500/{round_num}"
        new_fid = fid(round_dir)
        new_value = input_list[:5] + [new_fid]
        self.budget_fid[new_key] = new_value

        # Store the budget-fid in a txt
        output_dir = "./output_fid"
        os.makedirs(output_dir, exist_ok=True)

        # Generate the filename using the current date
        current_date = datetime.now().strftime("%Y-%m-%d")  # Format: YYYY-MM-DD
        output_file = os.path.join(output_dir, f"artbench-1500.txt")
        # output_file = os.path.join(output_dir, f"Portrait-RL-L.txt")
        # output_file = os.path.join(output_dir, f"Artbench-RL-L.txt")
    ...
    def get_image_quality(self, history_rewards, history_budgets, state):
        ...
        if t_l == 0:
            if length == 0:
                ...
                return 100/fid_result                      # normalize FID value
        else:
...
#                 fid_result = 24  # a default FID, will be updated
    ...
    if mode == 0:
        ...
        if os.path.exists(output_file):
            ...
            with open(output_file, 'r') as file:
                ...
                distribution = list(map(float, first_line.split()))
        ...
        budget_distribution = budget * np.array(distribution)
    elif mode == 1:
...
def launch_training(round_num, train_dir, num_data):
    ...
    os.environ["TRAIN_EPOCH"] = str(int((last_checkpoint_number + 500) * num_gpus / num_data + 40))
